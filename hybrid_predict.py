...
from generate_steps import generate_steps, generate_steps_simple
...
def hybrid_predict(dataset, use_best_hit=False, small_model="Qwen2.5-7B", large_model="Qwen2.5-72B", embed_model="BAAI/bge-base-en-v1.5", rerank_model="BAAI/bge-reranker-large"):
    ...
    for item in tqdm(dev):
        ...
        # prompt = "Question:\n" + item["question"] + "\n\nSteps:\n" + steps + "\n\nCorpus:\n" + text + "Question:\n" + item["question"]
        instruction = "You are a helpful assistant. According to the given corpus and steps, answer the given question. Output the executing result of every step. Then output the final answer as short as possible."
        prompt = "----------Question----------\n" + item["question"] + "\n\n----------Steps----------\n" + steps + "----------Corpus----------\n" + text +  "\n\n----------Question----------\n" + item["question"]
