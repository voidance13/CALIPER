...
def retrieve1(retrieved_path, embed_model, rerank_model, dataset):
    ...
    for q in tqdm(questions):
        ...
        retriever = ContextualCompressionRetriever(
            base_compressor=r, base_retriever=faiss_retriever
        )
...
def normal_predict(dataset, use_best_hit=False, model_name="Qwen2.5-7B", embed_model="BAAI/bge-base-en-v1.5", rerank_model="BAAI/bge-reranker-large"):
    ...
    for item in tqdm(dev):
        ...
        try:
            if model_name in ["Qwen2.5-7B-ft", "Qwen2.5-0.5B", "Qwen2.5-0.5B-ft", "all-Qwen2.5-7B-ft"]:
                ...
                previous_answer = parse_answer(res1)
                ...
                if ("Correct" in reflection or "correct" in reflection) and "incorrect" not in reflection and "Incorrect" not in reflection:
                    ...
                    response = client.chat.completions.create(
                        model=model_name,
                        messages=[
                            {"role": "system", "content": "According to the given question and the reasoning steps, output the complete final answer as short as possible. Specially, if the given question is a yes-no question, just output \"yes\" or \"no\"."},
                            {"role": "user", "content": "Question:\n" + item["question"] + "\n\nCorpus:\n" + text + "\n\n" + reflection + "\n\nQuestion:\n" + item["question"]},
                            # {"role": "assistant", "content": res1},
                            # {"role": "user", "content": reflection + "\n\nOutput the new final answer as short as possible."}
                        ],
                    )
    ...
    if use_best_hit and model_name == "Qwen2.5-7B":
        with open(f"data/{dataset}/normal_7b_ref_pred_use_best_hit.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit and model_name == "Qwen2.5-7B-ft":
        with open(f"data/{dataset}/normal_7bft_ref_pred_use_best_hit.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit and model_name == "Qwen2.5-0.5B":
        with open(f"data/{dataset}/normal_05b_ref_pred_use_best_hit.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit and model_name == "Qwen2.5-0.5B-ft":
        with open(f"data/{dataset}/normal_05bft_ref_pred_use_best_hit.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit == False and model_name == "Qwen2.5-7B":
        with open(f"data/{dataset}/normal_7b_ref_pred.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit == False and model_name == "Qwen2.5-7B-ft":
        with open(f"data/{dataset}/normal_7bft_ref_pred.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit and model_name == "Qwen2.5-72B":
        with open(f"data/{dataset}/normal_72b_ref_pred_use_best_hit.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit == False and model_name == "Qwen2.5-72B":
        with open(f"data/{dataset}/normal_72b_ref_pred.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit and model_name == "deepseek-r1":
        with open(f"data/{dataset}/normal_ds_r1_ref_pred_use_best_hit.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit == False and model_name == "deepseek-r1":
        with open(f"data/{dataset}/normal_ds_r1_ref_pred.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit and model_name == "deepseek-v3":
        with open(f"data/{dataset}/normal_ds_v3_ref_pred_use_best_hit.json","w") as f:
            json.dump(answers, f, indent=4)
    elif use_best_hit == False and model_name == "deepseek-v3":
        with open(f"data/{dataset}/normal_ds_v3_ref_pred.json","w") as f:
            json.dump(answers, f, indent=4)

    eval_res = evaluate(answers)

    if use_best_hit and model_name == "Qwen2.5-7B":
        with open(f"data/{dataset}/eval_normal_7b_ref_pred_use_best_hit.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit and model_name == "Qwen2.5-7B-ft":
        with open(f"data/{dataset}/eval_normal_7bft_ref_pred_use_best_hit.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit and model_name == "Qwen2.5-0.5B":
        with open(f"data/{dataset}/eval_normal_05b_ref_pred_use_best_hit.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit and model_name == "Qwen2.5-0.5B-ft":
        with open(f"data/{dataset}/eval_normal_05bft_ref_pred_use_best_hit.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit == False and model_name == "Qwen2.5-7B":
        with open(f"data/{dataset}/eval_normal_7b_ref_pred.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit == False and model_name == "Qwen2.5-7B-ft":
        with open(f"data/{dataset}/eval_normal_7bft_ref_pred.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit and model_name == "Qwen2.5-72B":
        with open(f"data/{dataset}/eval_normal_72b_ref_pred_use_best_hit.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit == False and model_name == "Qwen2.5-72B":
        with open(f"data/{dataset}/eval_normal_72b_ref_pred.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit and model_name == "deepseek-r1":
        with open(f"data/{dataset}/eval_normal_ds_r1_ref_pred_use_best_hit.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit == False and model_name == "deepseek-r1":
        with open(f"data/{dataset}/eval_normal_ds_r1_ref_pred.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit and model_name == "deepseek-v3":
        with open(f"data/{dataset}/eval_normal_ds_v3_ref_pred_use_best_hit.json","w") as f:
            json.dump(eval_res, f, indent=4)
    elif use_best_hit == False and model_name == "deepseek-v3":
        with open(f"data/{dataset}/eval_normal_ds_v3_ref_pred.json","w") as f:
            json.dump(eval_res, f, indent=4)
